...
/* @var $model common\models\Accomodation */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="card-body accomodation-form">
            ...
            <div class="form-group bmd-form-group">
                <div id="imagePriview">
                    ...
                    if (isset($model->id) && $model->id > 0 && isset($model->image) && $model->image !== "") {
                        $imgPath = ((yii\helpers\Url::base())) . '/../uploads/accomodation/' . $model->id . '/image/' . $model->image;
                    } else {
                    ...
                    if ($images != NULL) {
                        foreach ($images as $image) {
                            if (isset($model->id) && $model->id > 0 && isset($model->gallery) && $model->gallery !== "") {
                                $imgPath = ((yii\helpers\Url::base())) . '/../uploads/accomodation/' . $model->id . '/gallery/' . $image;
                            } else {
                                $imgPath = Yii::$app->request->baseUrl . '/img/no-image.jpg';
                            }
                            $delete_url = Yii::$app->request->baseUrl . '/accomodation/gallery-delete?id=' . $model->id . '&item=' . $image;
