...
/* @var $searchModel common\models\SystemConfigurationSearch */
...
<div class="content system-configuration-index">
...
//                                            'id',
                                    [
                                        ...
                                        'value' => function ($data) {
                                            if ($data->language != '') {
                                                $html = [];
                                                $exp_value = explode(',', $data->language);
                                                if ($exp_value != NULL) {
                                                    foreach ($exp_value as $exp_val) {
                                                        $html [] = \common\models\Language::findOne(['id' => $exp_val])->name;
                                                    }
                                                }
                                                if ($html != NULL) {
                                                    return implode(',', $html);
                                    ...
                                    [
                                        ...
                                        'value' => function ($data) {
                                            if ($data->currency != '') {
                                                $html = [];
                                                $exp_value = explode(',', $data->currency);
                                                if ($exp_value != NULL) {
                                                    foreach ($exp_value as $exp_val) {
                                                        $html[] = \common\models\Currency::findOne(['id' => $exp_val])->name;
                                                    }
                                                }
                                                if ($html != NULL) {
                                                    return implode(',', $html);
