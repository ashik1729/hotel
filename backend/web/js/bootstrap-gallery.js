...
$(document).ready(function () {
    ...
    $(document).keydown(function (e) {
        if ($('#bootstrap-gallery').length != 0 && $('#bootstrap-gallery').is(':visible')) {
            if (e.which == 37 && $('#bootstrap-gallery-prev').is(':visible'))
                $('#bootstrap-gallery-prev').trigger('click');
            else if (e.which == 39 && $('#bootstrap-gallery-next').is(':visible'))
                $('#bootstrap-gallery-next').trigger('click');
            else
                return;
...
function ExpandImage(link) {
    ...
    if ($(link).find('img').length == 0) {
        ...
        if (!title)
            title = $('a[href="' + url + '"]').filter(gallerySelector).has('img').find('img').attr('alt');

        // If not found, title is the "alt" attribute of existing image in page
        if (!title)
            title = $('img[src="' + url + '"]').attr('alt');
        ...
        if (gallery)
            thumbnails = thumbnails.filter('[data-gallery="' + gallery + '"]')
        else
            thumbnails = thumbnails.filter(':not([data-gallery])');
        ...
        if (pos > 0) {
            ...
            $('a#bootstrap-gallery-prev').off('click').click(function () {
                ExpandImage(prev);
                return false;
            });
        } else {
        ...
        if (pos < thumbnails.length - 1) {
            ...
            $('a#bootstrap-gallery-next').off('click').click(function () {
                ExpandImage(next);
                return false;
            });
        } else {
...
function GetGalleryHTML() {
    return "<div class='modal fade' id='bootstrap-gallery' tabindex='-1' role='dialog' aria-hidden='true'>" +
            ...
            "<a href='' class='modal-button' data-dismiss='modal'><i class='fa fa-times'></i></a>" +
            "<a href='' id='bootstrap-gallery-extend' class='modal-button' target='_blank'><i class='fa fa-expand-alt'></i></a>" +
            "<h5 class='modal-title'></h5>" +
            "</div>" +
            "<div class='modal-body'>" +
            "<img src='' />" +
            "<a id='bootstrap-gallery-prev' class='carousel-control-prev' href='#' data-slide='prev'><i class='fa fa-chevron-left'></i></a>" +
            "<a id='bootstrap-gallery-next' class='carousel-control-next' href='#' data-slide='next'><i class='fa fa-chevron-right'></i></a>" +
            "</div>" +
            "</div>" +
            "</div>" +
            "</div>";
